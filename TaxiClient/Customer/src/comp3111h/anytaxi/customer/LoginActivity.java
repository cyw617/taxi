...
public class LoginActivity extends ActionBarActivity {
	...
	private void exit() {
		showAlertDialog(this, getString(R.string.quit_Message),
	...
	/**
	 * @param context
	 *            The current context of UI thread
	 * @param msg
	 *            Message to show in the body of dialog
	 * @param posButtonText
	 *            Text of positive button
	 * @param negButtonText
	 *            Text of negative button
	 * @param posListener
	 *            Click handler of positive button
	 * @param negListener
	 *            Click handler of negative button
	 * 
	 */
	public void showAlertDialog(Context context, String msg,
		...
		new AlertDialog.Builder(context).setMessage(msg)
				.setPositiveButton(posButtonText, posListener)
				.setNegativeButton(negButtonText, negListener).show();
	}
	...
	private class CheckLoginTask extends AsyncTask<Void, Void, Customer> {
		...
		protected Customer doInBackground(Void... params) {
			String accountName = Utils.getPreference(context,
					Utils.PREFS_ACCOUNT_KEY, null);
			if (accountName == null) {
				return null;
			}
		...
		protected void onPostExecute(Customer result) {
			...
			if (exceptionThrown != null) {
				CloudEndpointUtils.logAndShow(LoginActivity.this, TAG,
						exceptionThrown);
				return;
			}
			if (result == null) {
				Log.i(TAG,
						"No user information in database: redirect to RegisterActivity.");
				...
				Toast.makeText(this.context, "Welcome, " + result.getName(),
