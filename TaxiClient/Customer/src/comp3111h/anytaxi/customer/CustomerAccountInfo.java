...
public class CustomerAccountInfo {
	...
	static void setAccountInfo(Intent myIntent) {
		CustomerAccountInfo.accountName = myIntent.getExtras().getString(
				AccountManager.KEY_ACCOUNT_NAME);
		if (CustomerAccountInfo.accountName != null) {
			CustomerAccountInfo.credential
					.setSelectedAccountName(CustomerAccountInfo.accountName);

			AnyTaxi.Builder endpointBuilder = new AnyTaxi.Builder(
					AndroidHttp.newCompatibleTransport(), new JacksonFactory(),
					null);
			CustomerAccountInfo.endpoint = CloudEndpointUtils.updateBuilder(
