...
public class RequestActivity extends ActionBarActivity implements
	...
	//Just different store format of mLatLng
	float latCurrent;
	float lntCurrent;
...
<<<<<<< HEAD
    ...
    private class EndpointsTask extends AsyncTask<Void, Void, Driver> {
		...
		protected Driver doInBackground(Void... params) {
			Driver driver = null;
			try {
				GetDriver task = endpoint.getDriver(email);
				driver = task.execute();
		...
		protected void onPostExecute(Driver result) {
			if (exceptionThrown != null)
				Log.e(TrackingActivity.class.getName(), 
						"Exception when submitting data", exceptionThrown);

			try {
				Log.e(TrackingActivity.class.getName(), result.getLoc().toPrettyString());
			} catch (Exception e) {
